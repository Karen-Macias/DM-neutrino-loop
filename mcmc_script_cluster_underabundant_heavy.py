...
#         lnlike_nu = 0
    ...
    if ysv_model >= y2:
        if -2.0292883347469104<=theta[0]<=-1.5023866744647538:
            lnlike_sv = -0.5*np.sum(((y2-3*U_e3**4*ysv_model)/(1/1.645*(1.0/f_fraction)**2*swave_sigmav_int(theta[0])))**2)  #90% CL = 1.645 sigma
...
filename = "scalar_dm_underabundant_heavy_result.h5"
