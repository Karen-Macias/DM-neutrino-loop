...
#     else:
#         lnlike_ra = 0
    lnlike_ra = -0.5*((0.12-yra_model)/0.0012)**2*np.heaviside(yra_model-0.12,0.12)
    ...
    if ysv_model >= y2:
        if -2.0292883347469104<=theta[0]<=-1.5023866744647538:
            lnlike_sv = -0.5*np.sum(((y2-3*U_e1**4*ysv_model)/(1/1.645*(1.0/f_fraction)**2*swave_sigmav_int(theta[0])))**2)  #90% CL = 1.645 sigma
...
#         lnlike_D = 0 
    ...
    if 10**theta[1]+10**theta[3]<m_K:
        ...
        if yK_model >= y2:
            lnlike_Ke = -0.5*np.sum(((y2-yK_model*U_e1)/(1/1.645*yerr_ke))**2) 
            lnlike_Km = -0.5*np.sum(((y2-yK_model*U_m1)/(1/1.645*yerr_km))**2) #90% CL
    ...
    ln_like_total = lnlike_ra + lnlike_u + lnlike_sv + lnlike_neff + lnlike_nu + lnlike_z + lnlike_K
    return ln_like_total, lnlike_ra, lnlike_u, lnlike_sv, lnlike_neff, lnlike_nu, lnlike_z, lnlike_K, yra_model, ysv_model, yu_model, ynu_model, yz_model, yK_model
...
def ln_prior(theta):
    ...
    if 10**mn>10**mphi1>=mphi2_lin>=10**mx and -4<=mx<=8 and -4<=mphi1<=8 and -16<=del_mphi<=0 and -4<=mn<=8 and -7<=lam<=np.log10(4*np.pi) and -7<=Ynu<=np.log10(np.sqrt(4*np.pi)):
        return 0                          #log flat prior
    return -np.inf # log(0)
...
ndim = 6         #N-dimensinal parameter space
...
filename = "scalar_dm_underabundant_light_result.h5"
...
yerr1 = y_dm#+0.0012         #upper limit 68% C.L. Planck TT, TE, EE + lowE + lensing
...
pos = [theta_i + 0.1*np.random.randn(ndim) for i in range(nwalkers)]
